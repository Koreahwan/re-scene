"""
Phase 1 Independent Audit Regression Tests
Ensures all 5 defects identified during the independent audit remain permanently fixed:
1. Magazine comment creation returns HTTP 201 without 500 unwrap crash.
2. Magazine comments follow common spoiler masking policies (unverified comments are masked).
3. Magazine routes reject non-article IDs with 404.
4. Model inspection fails closed on empty JSON output (status != VERIFIED).
5. Model inspection fails closed on unsupported film / fake evidence IDs (status != VERIFIED).
"""
import uuid
...
def test_magazine_comment_create_returns_success(article):
    """Magazine comment creation must return 201 without 500 error."""
...
def test_magazine_unverified_comment_is_masked(article):
    """Magazine comment reading must enforce spoiler masking on unverified comments."""
...
def test_magazine_route_rejects_non_article(monkeypatch):
    """Magazine endpoints must reject non-magazine posts (e.g. FAN_THEORY) with 404."""
    monkeypatch.setattr(settings, "PHASE1_SUBMISSION_PROFILE_ENABLED", True)
...
async def test_unsubstantiated_model_output_cannot_be_verified(monkeypatch, work, reply):
    """AI inspection must fail closed when model output is empty or references fake evidence/unsupported films."""
