...
CHECKPOINT_PATH = Path(r"data\manifests\preprocessing_checkpoint_v2_gemini36.json")

CANONICAL_ASSET_SHA256 = "8bb196344ab03a29e90dd62a08a318329560582c69fafe36aac4354d40f64948"
MODEL_ID = "gemini-3.6-flash"
GCP_LOCATION = "global"
PROMPT_VERSION = "v2.0-observable"
SCHEMA_VERSION = "v2.0"
...
async def run_uniform_gemini36_preprocessing():
    from google import genai
    from PIL import Image

    print("===================================================================")
    print(f"   STARTING UNIFORM GEMINI 3.6 FULL-FILM PREPROCESSING ({DATASET_VERSION})")
    print("===================================================================")
    ...
    for chunk_idx in range(total_chunks):
        ...
        temp_dir = Path(f"scratch/v2_frames_{chunk_id}")
        frames = extract_sample_frames(VIDEO_PATH, start_sec, duration_sec, temp_dir)
        print(f"Extracted {len(frames)} keyframes.")

        images = [Image.open(f) for f in frames[:5]]  # Compact 5 keyframes per 2-min chunk
...
"""
        ...
        while retry_count < max_retries and not success:
            try:
                ...
                retry_count += 1
                print(f"Attempt {retry_count} failed with error: {e}. Retrying...")
                time.sleep(2.0)

        if not success:
            print(f"ERROR: Failed chunk {chunk_id} after {max_retries} attempts.")
            checkpoint["failed_chunks"][chunk_id] = {
                "start_sec": start_sec,
        ...
        scene_id = f"scene-tbw-v2-{chunk_idx+1:03d}"
        ...
        for idx, ev in enumerate(validated.events, 1):
            event_records.append(Event(
                event_id=f"ev-{scene_id}-{idx:02d}",
                scene_id=scene_id,
                movie_id="the-bat-whispers-1930",
        ...
        for idx, f in enumerate(validated.facts, 1):
            fact_records.append(Fact(
                fact_id=f"fact-{scene_id}-{idx:02d}",
                scene_id=scene_id,
                movie_id="the-bat-whispers-1930",
                timestamp_ms=start_ms + (idx * 5000),
        ...
        # Ingest into ClickHouse via MCP with DATASET_VERSION isolation
        ...
        VALUES ('{scene_record.scene_id}', '{scene_record.movie_id}', {scene_record.start_ms}, {scene_record.end_ms}, '{loc}', {chars}, {objs}, '{summary}', '{DATASET_VERSION}');
        """
        ...
        checkpoint["processed_chunks"][chunk_id] = {
            ...
            "characters_count": len(scene_record.characters),
            "objects_count": len(scene_record.objects),
            "prompt_tokens": p_tokens,
            "candidate_tokens": c_tokens,
            "latency_sec": round(t1 - t0, 3),
            "cost_krw": round(chunk_cost_krw, 4),
            "retries": retry_count,
            "status": "COMPLETED",
            "extracted_at": datetime.now(timezone.utc).isoformat(),
        }
        save_checkpoint(checkpoint)
        print(f"Saved checkpoint for {chunk_id} ({len(event_records)} events, {len(fact_records)} facts).")

    print("\n=======================================================")
    print(f"      UNIFORM GEMINI 3.6 FULL-FILM PREPROCESSING COMPLETED")
    print("=======================================================")
    print(f"Dataset Version: {DATASET_VERSION}")
    print(f"Canonical Asset SHA: {CANONICAL_ASSET_SHA256}")
    print(f"Total Chunks Processed: {len(checkpoint['processed_chunks'])} / {total_chunks}")
    print(f"Failed Chunks: {len(checkpoint['failed_chunks'])}")
    print(f"Total Real Model Calls: {checkpoint['total_model_calls']}")
...
if __name__ == "__main__":
    import asyncio
    asyncio.run(run_uniform_gemini36_preprocessing())
